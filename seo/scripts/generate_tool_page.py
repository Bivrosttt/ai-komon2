...
document.querySelectorAll('[data-risk-item]').forEach(item => item.addEventListener('change', updateRisk));'''
    ...
    if tool_type == "text-counter":
        ...
        setup = "document.getElementById('tool-text').addEventListener('input', event => { document.getElementById('tool-result').textContent = event.target.value.length.toLocaleString('ja-JP') + '文字'; });"
